import { Home, Search, Plus, MessageCircle, User, Compass, Heart, Settings, TrendingUp, Shield, ChevronLeft, ChevronRight, Bell } from "lucide-react";
...
export default function DesktopSidebar() {
  ...
  return (
    <aside
      className={`hidden md:flex fixed left-0 top-0 bottom-0 z-50 flex-col border-r border-border bg-sidebar transition-all duration-300 ${
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 pt-4 pb-2 mb-2">
        ...
        <motion.button
          ...
          className="h-7 w-7 rounded-md flex items-center justify-center hover:bg-sidebar-accent transition-colors flex-shrink-0"
        >
      ...
      <nav className="flex flex-col gap-0.5 flex-1 px-2">
        ...
        <motion.button
          ...
          className={`mt-3 flex items-center justify-center gap-2 rounded-xl gradient-primary py-3 text-sm font-bold text-primary-foreground pulse-glow ${
            collapsed ? "px-0" : ""
          }`}
        >
      ...
      <div className="border-t border-border px-2 py-2">
        {bottomItems.map((item) => (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate(item.path)}
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-sidebar-foreground/50 hover:text-sidebar-foreground transition-colors ${
              collapsed ? "justify-center" : ""
            }`}
            title={collapsed ? item.label : undefined}
          >
